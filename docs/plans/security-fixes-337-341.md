...
2. **#338** — Enforce `ProjectUser` membership check in `TimeEntryService.CreateTimeEntry`/`UpdateTimeEntry` before assigning `ProjectId`. Reuse `ProjectService.GetAssignedProjects` predicate. Unit test: unassigned user rejected on create + update. — **PR open**
...
- Note: this plan file was independently created on both the #337 and #338 branches (each branched from `main` before the other merged) — expect a trivial merge conflict here when both land; take the union of status updates.
