...
3. **#341** — Three small endpoint auth fixes (grouped by the issue itself):
   ...
   — **PR open**. Along the way, found and filed a separate pre-existing bug (#377, High priority): `ProjectDetailPage.razor` breaks for all non-admin users because `Reload()` atomically awaits an admin-only `/api/admin/users` call — not caused by this PR, not fixed by it either, deliberately left for its own PR.
...
- This plan file was independently created on the #337, #338, and #341 branches (each branched from `main` before the others merged) — expect trivial merge conflicts here; take the union of status updates.
