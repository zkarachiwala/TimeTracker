...
- Test target: local dev server on port 7007 (full suite pre-push; CI smoke test only — see ADR-016)
...
Migrated `TimeTracker.Playwright` from NUnit to xUnit; added `TimeTracker.ComponentTests` (bUnit) for Blazor component tests. See [ADR-026](decisions.md#adr-026-xunit-over-nunit-for-playwright--new-bunit-component-layer).
...
Eliminated `showcase-app.css` drift by unifying CSS via MSBuild; added `ShowcaseFixture` + `ShowcaseTests` to smoke-test every showcase page. See [ADR-027](decisions.md#adr-027-showcase-css-unified-via-msbuild).
