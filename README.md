...
This enables the pre-push hook, which runs the full Playwright test suite automatically before every push when app code has changed. The hook exists because there is no staging environment — authenticated E2E tests cannot run in CI against production, so the pre-push hook is the gate. See [ADR-016](docs/decisions.md#adr-016-playwright--full-suite-pre-push-ci-smoke-test-only) for the full decision including how this changes if a staging environment is added.
