...
| `docs/decisions.md` | Add **ADR-017**: NUnit → xUnit migration — decision, context, consequences. Update ADR-010/ADR-016 where they reference `[SetUpFixture]`, `[OneTimeSetUp]`, `Assert.Ignore()` (annotate as deprecated patterns replaced in ADR-017). Update index table. |
